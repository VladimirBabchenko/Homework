...
export default class BattleGround extends Squad {
    constructor(squad, landscape) {
        super();
        this.battleField = [];
        if (squad) this.addToBattleGround(squad);
        this.addToDomField(landscape);
    }

    addToDomField(landscape) {
        ...
        this.battleField.forEach(squad => teamsBlock.appendChild(squad.teamBlock));
    
        wrapper.append(teamsBlock, fieldScoreWrapper);
        fieldScoreWrapper.append(fieldBattle, scoreBoard);
    };

    addToBattleGround(squad) {
    ...
    attacked() {
        ...
        if(defendingWarrior.currentHealth <= 0) {
            ...
            throw new Error (helpers.showKilling(defendingWarrior));
        }
    ...
    fight(team2) {
        ...
        setTimeout(function callback() {
            try {
                ...
                setTimeout(callback, helpers.getFightingInterval());
            }
        }, 1000)
    };

    defineWarriorIndex(num) {
